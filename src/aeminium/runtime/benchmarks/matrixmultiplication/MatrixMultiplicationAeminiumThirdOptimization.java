...
public class MatrixMultiplicationAeminiumThirdOptimization {
	...
	public static void main(String args[]) {
		...
		if (n != p) {
			...
			Task init1=createMatrix(Runtime.NO_PARENT, Runtime.NO_DEPS);
			prev1.add(init1);
			// second[c][d] = in.nextInt();

			multiplyTask(Runtime.NO_PARENT, prev1);
	...
	private static Task createMatrix(Task current, Collection<Task> prev) {
		Task task = rt.createNonBlockingTask(new Body() {
			@Override
			public void execute(Runtime rt, Task current) {
				...
				for (int c = 0; c < m; c = c + step1) {
					Task init1 = createFirstMatrixTask(current, Runtime.NO_DEPS, c, (c + step1));
					prev1.add(init1);
				}
				...
				for (int c = 0; c < p; c = c + step2) {
					Task init2 = createSecondMatrixTask(current, prev1, c, (c + step2));
					prev2.add(init2);
				}
...
//				multiplyMatrixTask(current, Runtime.NO_DEPS, 1680, 1920);
				...
				for (int c = 0; c < m; c = c + step3) {
					multiplyMatrixTask(Runtime.NO_PARENT, Runtime.NO_DEPS, c, (c + step3));
				}
