...
public class AeKDTree {
	...
	public void createSubTrees(final Point[] points, final int depth, Runtime rt, Task parent) {
		int median = (points.length - 1) / 2;
		if (Benchmark.useThreshold ? depth <= Point.DEFAULT_THRESHOLD : !rt.parallelize(parent)) createSubTreesInPar(points, depth, rt, parent, median);
		else createSubTreesInSeq(points, depth, rt, parent, median);
	}
	...
	protected void createSubTreesInPar(final Point[] points, final int depth, Runtime rt, Task parent, final int median) {
		// Create childA and childB recursively.
		if (median > 0) {
