...
public class FjJacobi {
	...
	static final class Driver extends RecursiveAction {
		...
		Driver(double[][] A, double[][] B, int firstRow, int lastRow, int firstCol, int lastCol, int steps, int leafs) {
			this.A = A;
			this.B = B;
		...
		MatrixTree build(double[][] a, double[][] b, int lr, int hr, int lc, int hc, int leafs) {
			...
			if (rows * cols <= leafs) {
				...
				return new FourNode(build(a, b, lr, mr, lc, mc, leafs), build(a, b, lr, mr, mc + 1, hc, leafs), build(a, b, mr + 1, hr, lc, mc, leafs), build(
						a, b, mr + 1, hr, mc + 1, hc, leafs));
			} else if (cols >= rows) {
